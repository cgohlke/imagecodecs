...
NC4_get_var_chunk_cache(
    int ncid, int varid, size_t *sizep, size_t *nelemsp, float *preemptionp)
{
    NC *nc;
    NC_GRP_INFO_T *grp;
    NC_FILE_INFO_T *h5;
...
nc_get_var_chunk_cache_ints(
    int ncid, int varid, int *sizep, int *nelemsp, int *preemptionp)
{
    size_t real_size, real_nelems;
    float real_preemption;
    int ret;

    if ((ret = NC4_get_var_chunk_cache(
    ...
    unsigned int *params)
{
    ...
    if (dimidsp)
        for (d = 0; d < var->ndims; d++) dimidsp[d] = var->dimids[d];
    if (nattsp)
    ...
    if (shufflep) {
        retval =
            nc_inq_var_filter_info(ncid, varid, H5Z_FILTER_SHUFFLE, 0, NULL);
        if (retval && retval != NC_ENOFILTER)
            return retval;
        *shufflep = (retval == NC_NOERR ? 1 : 0);
    }
    if (fletcher32p) {
        retval = nc_inq_var_filter_info(
            ncid, varid, H5Z_FILTER_FLETCHER32, 0, NULL);
        if (retval && retval != NC_ENOFILTER)
            return retval;
    ...
    if (!var->no_fill && fill_valuep) {
        ...
        if (var->fill_value) {
            ...
            if ((retval = NC_copy_data(
                     h5->controller, xtype, var->fill_value, 1, fill_valuep)))
                return retval;
        } else {
            if ((retval =
                     nc4_get_default_fill_value(var->type_info, fill_valuep)))
                return retval;
        }
    }
...
NC4_var_par_access(int ncid, int varid, int par_access)
{
    ...
    LOG(
        (1,
         "%s: ncid 0x%x varid %d par_access %d",
         __func__,
         ncid,
         varid,
         par_access));
    ...
    int nsd)
{
    ...
    const double bit_per_dgt =
        M_LN10 /
        M_LN2; /* 3.32 [frc] Bits per decimal digit of precision  = log2(10) */
    const double dgt_per_bit =
        M_LN2 / M_LN10; /* 0.301 [frc] Decimal digits per bit of precision =
                           log10(2) */
    ...
    double mss_val_cmp_dbl; /* Missing value for comparison to double precision
                               values */
    float mss_val_cmp_flt; /* Missing value for comparison to single precision
                              values */
    int bit_xpl_nbr_zro; /* [nbr] Number of explicit bits to zero */
    int dgt_nbr; /* [nbr] Number of digits before decimal point */
    int qnt_pwr; /* [nbr] Power of two in quantization mask: qnt_msk =
                    2^qnt_pwr */
    int xpn_bs2; /* [nbr] Binary exponent xpn_bs2 in val = sign(val) *
                    2^xpn_bs2 * mnt, 0.5 < mnt <= 1.0 */
    size_t idx;
    ...
    unsigned short prc_bnr_xpl_rqr; /* [nbr] Explicitly represented binary
                                       digits required to retain */
    ...
    *range_error = 0;
    LOG(
        (3,
         "%s: len %d src_type %d dest_type %d",
         __func__,
         len,
         src_type,
         dest_type));
    ...
    if (quantize_mode != NC_NOQUANTIZE) {
        ...
        /* Set parameters used by BitGroom and BitRound here, outside value
           loop. Equivalent parameters used by GranularBR are set inside value
           loop, since keep bits and thus masks can change for every value. */
        if (quantize_mode == NC_QUANTIZE_BITGROOM || quantize_mode == NC_QUANTIZE_BITROUND) {
            if (quantize_mode == NC_QUANTIZE_BITGROOM) {
                /* BitGroom interprets nsd as number of significant decimal
                 * digits Must convert that to number of significant bits to
                 * preserve How many bits to preserve? Being conservative, we
                 * round up the exact binary digits of precision. Add one
                 * because the first bit is implicit not explicit but corner
                 * cases prevent our taking advantage of this. */
                prc_bnr_xpl_rqr = (unsigned short)ceil(nsd * bit_per_dgt) + 1;

            } else if (quantize_mode == NC_QUANTIZE_BITROUND) {
                /* BitRound interprets nsd as number of significant binary
                 * digits (bits) */
    ...
    */
    switch (src_type) {
        ...
        case NC_BYTE:
            switch (dest_type) {
                case NC_BYTE:
                    for (bp = (signed char *)src, bp1 = dest; count < len;
                         count++)
                        *bp1++ = *bp++;
                    break;
                case NC_UBYTE:
                    for (bp = (signed char *)src, ubp = dest; count < len;
                         count++) {
                        if (*bp < 0)
                            (*range_error)++;
                    ...
                    break;
                case NC_SHORT:
                    for (bp = (signed char *)src, sp = dest; count < len;
                         count++)
                        *sp++ = *bp++;
                    break;
                case NC_USHORT:
                    for (bp = (signed char *)src, usp = dest; count < len;
                         count++) {
                        if (*bp < 0)
                            (*range_error)++;
                    ...
                    break;
                case NC_INT:
                    for (bp = (signed char *)src, ip = dest; count < len;
                         count++)
                        *ip++ = *bp++;
                    break;
                case NC_UINT:
                    for (bp = (signed char *)src, uip = dest; count < len;
                         count++) {
                        if (*bp < 0)
                            (*range_error)++;
                        *uip++ = (unsigned int)*bp++;
                    ...
                    break;
                case NC_INT64:
                    for (bp = (signed char *)src, lip = dest; count < len;
                         count++)
                        *lip++ = *bp++;
                    break;
                case NC_UINT64:
                    for (bp = (signed char *)src, ulip = dest; count < len;
                         count++) {
                        if (*bp < 0)
                            (*range_error)++;
                    ...
                    break;
                case NC_FLOAT:
                    for (bp = (signed char *)src, fp = dest; count < len;
                         count++)
                        *fp++ = *bp++;
                    break;
                case NC_DOUBLE:
                    for (bp = (signed char *)src, dp = dest; count < len;
                         count++)
                        *dp++ = *bp++;
                    break;
                default:
        ...
        case NC_UBYTE:
            switch (dest_type) {
                case NC_BYTE:
                    for (ubp = (unsigned char *)src, bp = dest; count < len;
                         count++) {
                    ...
                    break;
                case NC_SHORT:
                    for (ubp = (unsigned char *)src, sp = dest; count < len;
                         count++)
                        *sp++ = *ubp++;
                    break;
                case NC_UBYTE:
                    for (ubp = (unsigned char *)src, ubp1 = dest; count < len;
                         count++)
                        *ubp1++ = *ubp++;
                    break;
                case NC_USHORT:
                    for (ubp = (unsigned char *)src, usp = dest; count < len;
                         count++)
                        *usp++ = *ubp++;
                    break;
                case NC_INT:
                    for (ubp = (unsigned char *)src, ip = dest; count < len;
                         count++)
                        *ip++ = *ubp++;
                    break;
                case NC_UINT:
                    for (ubp = (unsigned char *)src, uip = dest; count < len;
                         count++)
                        *uip++ = *ubp++;
                    break;
                case NC_INT64:
                    for (ubp = (unsigned char *)src, lip = dest; count < len;
                         count++)
                        *lip++ = *ubp++;
                    break;
                case NC_UINT64:
                    for (ubp = (unsigned char *)src, ulip = dest; count < len;
                         count++)
                        *ulip++ = *ubp++;
                    break;
                case NC_FLOAT:
                    for (ubp = (unsigned char *)src, fp = dest; count < len;
                         count++)
                        *fp++ = *ubp++;
                    break;
                case NC_DOUBLE:
                    for (ubp = (unsigned char *)src, dp = dest; count < len;
                         count++)
                        *dp++ = *ubp++;
                    break;
                default:
        ...
        case NC_SHORT:
            switch (dest_type) {
                ...
                case NC_INT64:
                    ...
                    break;
                case NC_UINT64:
                    for (sp = (short *)src, ulip = dest; count < len;
                         count++) {
                        if (*sp < 0)
                            (*range_error)++;
        ...
        case NC_USHORT:
            switch (dest_type) {
                case NC_UBYTE:
                    for (usp = (unsigned short *)src, ubp = dest; count < len;
                         count++) {
                        if (*usp > X_UCHAR_MAX)
                            (*range_error)++;
                    ...
                    break;
                case NC_BYTE:
                    for (usp = (unsigned short *)src, bp = dest; count < len;
                         count++) {
                        if (*usp > X_SCHAR_MAX)
                            (*range_error)++;
                        *bp++ = (signed char)*usp++;
                    ...
                    break;
                case NC_SHORT:
                    for (usp = (unsigned short *)src, sp = dest; count < len;
                         count++) {
                        if (*usp > X_SHORT_MAX)
                            (*range_error)++;
                        *sp++ = (signed short)*usp++;
                    ...
                    break;
                case NC_USHORT:
                    for (usp = (unsigned short *)src, usp1 = dest; count < len;
                         count++)
                        *usp1++ = *usp++;
                    break;
                case NC_INT:
                    for (usp = (unsigned short *)src, ip = dest; count < len;
                         count++)
                        *ip++ = *usp++;
                    break;
                case NC_UINT:
                    for (usp = (unsigned short *)src, uip = dest; count < len;
                         count++)
                        *uip++ = *usp++;
                    break;
                case NC_INT64:
                    for (usp = (unsigned short *)src, lip = dest; count < len;
                         count++)
                        *lip++ = *usp++;
                    break;
                case NC_UINT64:
                    for (usp = (unsigned short *)src, ulip = dest; count < len;
                         count++)
                        *ulip++ = *usp++;
                    break;
                case NC_FLOAT:
                    for (usp = (unsigned short *)src, fp = dest; count < len;
                         count++)
                        *fp++ = *usp++;
                    break;
                case NC_DOUBLE:
                    for (usp = (unsigned short *)src, dp = dest; count < len;
                         count++)
                        *dp++ = *usp++;
                    break;
                default:
        ...
        case NC_UINT:
            switch (dest_type) {
                case NC_UBYTE:
                    for (uip = (unsigned int *)src, ubp = dest; count < len;
                         count++) {
                        if (*uip > X_UCHAR_MAX)
                            (*range_error)++;
                    ...
                    break;
                case NC_BYTE:
                    for (uip = (unsigned int *)src, bp = dest; count < len;
                         count++) {
                        if (*uip > X_SCHAR_MAX)
                            (*range_error)++;
                        *bp++ = (signed char)*uip++;
                    ...
                    break;
                case NC_SHORT:
                    for (uip = (unsigned int *)src, sp = dest; count < len;
                         count++) {
                        if (*uip > X_SHORT_MAX)
                            (*range_error)++;
                        *sp++ = (signed short)*uip++;
                    ...
                    break;
                case NC_USHORT:
                    for (uip = (unsigned int *)src, usp = dest; count < len;
                         count++) {
                        if (*uip > X_USHORT_MAX)
                            (*range_error)++;
                    ...
                    break;
                case NC_INT:
                    for (uip = (unsigned int *)src, ip = dest; count < len;
                         count++) {
                        if (*uip > X_INT_MAX)
                            (*range_error)++;
                        *ip++ = (int)*uip++;
                    ...
                    break;
                case NC_UINT:
                    for (uip = (unsigned int *)src, uip1 = dest; count < len;
                         count++) {
                        if (*uip > X_UINT_MAX)
                            (*range_error)++;
                        *uip1++ = *uip++;
                    ...
                    break;
                case NC_INT64:
                    for (uip = (unsigned int *)src, lip = dest; count < len;
                         count++)
                        *lip++ = *uip++;
                    break;
                case NC_UINT64:
                    for (uip = (unsigned int *)src, ulip = dest; count < len;
                         count++)
                        *ulip++ = *uip++;
                    break;
                case NC_FLOAT:
                    for (uip = (unsigned int *)src, fp = dest; count < len;
                         count++)
                        *fp++ = (float)*uip++;
                    break;
                case NC_DOUBLE:
                    for (uip = (unsigned int *)src, dp = dest; count < len;
                         count++)
                        *dp++ = *uip++;
                    break;
                default:
        ...
        case NC_INT64:
            switch (dest_type) {
                case NC_UBYTE:
                    for (lip = (long long *)src, ubp = dest; count < len;
                         count++) {
                    ...
                    break;
                case NC_BYTE:
                    for (lip = (long long *)src, bp = dest; count < len;
                         count++) {
                    ...
                    break;
                case NC_SHORT:
                    for (lip = (long long *)src, sp = dest; count < len;
                         count++) {
                    ...
                    break;
                case NC_USHORT:
                    for (lip = (long long *)src, usp = dest; count < len;
                         count++) {
                    ...
                    break;
                case NC_UINT:
                    for (lip = (long long *)src, uip = dest; count < len;
                         count++) {
                    ...
                    break;
                case NC_INT:
                    for (lip = (long long *)src, ip = dest; count < len;
                         count++) {
                    ...
                    break;
                case NC_INT64:
                    for (lip = (long long *)src, lip1 = dest; count < len;
                         count++)
                        *lip1++ = *lip++;
                    break;
                case NC_UINT64:
                    for (lip = (long long *)src, ulip = dest; count < len;
                         count++) {
                        if (*lip < 0)
                            (*range_error)++;
                    ...
                    break;
                case NC_FLOAT:
                    for (lip = (long long *)src, fp = dest; count < len;
                         count++)
                        *fp++ = (float)*lip++;
                    break;
                case NC_DOUBLE:
                    for (lip = (long long *)src, dp = dest; count < len;
                         count++)
                        *dp++ = (double)*lip++;
                    break;
                default:
        ...
        case NC_UINT64:
            switch (dest_type) {
                case NC_UBYTE:
                    for (ulip = (unsigned long long *)src, ubp = dest;
                         count < len;
                         count++) {
                        if (*ulip > X_UCHAR_MAX)
                            (*range_error)++;
                    ...
                    break;
                case NC_BYTE:
                    for (ulip = (unsigned long long *)src, bp = dest;
                         count < len;
                         count++) {
                        if (*ulip > X_SCHAR_MAX)
                            (*range_error)++;
                    ...
                    break;
                case NC_SHORT:
                    for (ulip = (unsigned long long *)src, sp = dest;
                         count < len;
                         count++) {
                        if (*ulip > X_SHORT_MAX)
                            (*range_error)++;
                    ...
                    break;
                case NC_USHORT:
                    for (ulip = (unsigned long long *)src, usp = dest;
                         count < len;
                         count++) {
                    ...
                    break;
                case NC_UINT:
                    for (ulip = (unsigned long long *)src, uip = dest;
                         count < len;
                         count++) {
                        if (*ulip > X_UINT_MAX)
                            (*range_error)++;
                    ...
                    break;
                case NC_INT:
                    for (ulip = (unsigned long long *)src, ip = dest;
                         count < len;
                         count++) {
                        if (*ulip > X_INT_MAX)
                            (*range_error)++;
                    ...
                    break;
                case NC_INT64:
                    for (ulip = (unsigned long long *)src, lip = dest;
                         count < len;
                         count++) {
                        if (*ulip > X_INT64_MAX)
                            (*range_error)++;
                    ...
                    break;
                case NC_UINT64:
                    for (ulip = (unsigned long long *)src, ulip1 = dest;
                         count < len;
                         count++)
                        *ulip1++ = *ulip++;
                    break;
                case NC_FLOAT:
                    for (ulip = (unsigned long long *)src, fp = dest;
                         count < len;
                         count++)
                        *fp++ = (float)*ulip++;
                    break;
                case NC_DOUBLE:
                    for (ulip = (unsigned long long *)src, dp = dest;
                         count < len;
                         count++)
                        *dp++ = (double)*ulip++;
                    break;
            ...
            switch (dest_type) {
                ...
                case NC_BYTE:
                    for (fp = (float *)src, bp = dest; count < len; count++) {
                        if (*fp > (double)X_SCHAR_MAX ||
                            *fp < (double)X_SCHAR_MIN)
                            (*range_error)++;
                        *bp++ = (signed char)*fp++;
                    }
                    break;
                case NC_SHORT:
                    for (fp = (float *)src, sp = dest; count < len; count++) {
                        if (*fp > (double)X_SHORT_MAX ||
                            *fp < (double)X_SHORT_MIN)
                            (*range_error)++;
                        *sp++ = (short)*fp++;
                    }
                    break;
                ...
                case NC_INT64:
                    ...
                    break;
                case NC_UINT64:
                    for (fp = (float *)src, ulip = dest; count < len;
                         count++) {
        ...
        case NC_DOUBLE:
            switch (dest_type) {
                case NC_UBYTE:
                    for (dp = (double *)src, ubp = dest; count < len;
                         count++) {
                ...
                case NC_SHORT:
                    ...
                    break;
                case NC_USHORT:
                    for (dp = (double *)src, usp = dest; count < len;
                         count++) {
                    ...
                    break;
                case NC_UINT:
                    for (dp = (double *)src, uip = dest; count < len;
                         count++) {
                ...
                case NC_INT:
                    ...
                    break;
                case NC_INT64:
                    for (dp = (double *)src, lip = dest; count < len;
                         count++) {
                    ...
                    break;
                case NC_UINT64:
                    for (dp = (double *)src, ulip = dest; count < len;
                         count++) {
                ...
                case NC_FLOAT:
                    for (dp = (double *)src, fp = dest; count < len; count++) {
                        if (isgreater(*dp, X_FLOAT_MAX) ||
                            isless(*dp, X_FLOAT_MIN))
                            (*range_error)++;
                        *fp++ = (float)*dp++;
                    }
                    break;
    ...
    if (quantize_mode == NC_QUANTIZE_BITGROOM) {
        if (dest_type == NC_FLOAT) {
            ...
            for (idx = 1L; idx < len; idx += 2L)
                if (op1.fp[idx] != mss_val_cmp_flt &&
                    u32_ptr[idx] != 0U) /* Never quantize upwards floating
            ...
            for (idx = 1L; idx < len; idx += 2L)
                if (op1.dp[idx] != mss_val_cmp_dbl &&
                    u64_ptr[idx] != 0ULL) /* Never quantize upwards floating
                                             point values of zero */
    ...
    if (quantize_mode == NC_QUANTIZE_BITROUND) {
        if (dest_type == NC_FLOAT) {
            ...
            for (idx = 0L; idx < len; idx++) {
                if (op1.fp[idx] != mss_val_cmp_flt) {
                    u32_ptr[idx] +=
                        msk_f32_u32_hshv; /* Add 1 to the MSB of LSBs, carry 1
                                             to mantissa or even exponent */
            ...
            for (idx = 0L; idx < len; idx++) {
                if (op1.dp[idx] != mss_val_cmp_dbl) {
                    u64_ptr[idx] +=
                        msk_f64_u64_hshv; /* Add 1 to the MSB of LSBs, carry 1
                                             to mantissa or even exponent */
        ...
        if (dest_type == NC_FLOAT) {
            ...
            for (idx = 0L; idx < len; idx++) {
                if ((val = op1.fp[idx]) != mss_val_cmp_flt &&
                    ...
                    /* 20211003 Continuous determination of dgt_nbr improves CR
                     * by ~10% */
                    dgt_nbr =
                        (int)floor(xpn_bs2 * dgt_per_bit + mnt_log10_fabs) +
                        1; /* DGG19 p. 4102 (8.67) */
                    qnt_pwr = (int)floor(
                        bit_per_dgt * (dgt_nbr - nsd)); /* DGG19 p. 4101 (7) */
                    prc_bnr_xpl_rqr =
                        mnt_fabs == 0.0
                            ? 0
                            : (unsigned short)abs(
                                  (int)floor(
                                      xpn_bs2 - bit_per_dgt * mnt_log10_fabs) -
                                  qnt_pwr); /* Protect against mnt = -0.0 */
                    prc_bnr_xpl_rqr--; /* 20211003 Reduce formula result by 1
                                          bit: Passes all tests, improves CR by
                                          ~10% */

                    bit_xpl_nbr_zro = BIT_XPL_NBR_SGN_FLT - prc_bnr_xpl_rqr;
                    msk_f32_u32_zro = 0U; /* Zero all bits */
                    msk_f32_u32_zro =
                        ~msk_f32_u32_zro; /* Turn all bits to ones */
                    /* Bit Shave mask for AND: Left shift zeros into bits to be
                     * rounded, leave ones in untouched bits */
                    msk_f32_u32_zro <<= bit_xpl_nbr_zro;
                    /* Bit Set   mask for OR:  Put ones into bits to be set,
                     * zeros in untouched bits */
                    msk_f32_u32_one = ~msk_f32_u32_zro;
                    msk_f32_u32_hshv = msk_f32_u32_one &
                                       (msk_f32_u32_zro >>
                                        1); /* Set one bit: the MSB of LSBs */
                    u32_ptr[idx] +=
                        msk_f32_u32_hshv; /* Add 1 to the MSB of LSBs, carry 1
                                             to mantissa or even exponent */
                    u32_ptr[idx] &= msk_f32_u32_zro; /* Shave it */
            ...
            for (idx = 0L; idx < len; idx++) {
                if ((val = op1.dp[idx]) != mss_val_cmp_dbl &&
                    ...
                    msk_f64_u64_zro =
                        ~msk_f64_u64_zro; /* Turn all bits to ones */
                    /* Bit Shave mask for AND: Left shift zeros into bits to be
                     * rounded, leave ones in untouched bits */
                    msk_f64_u64_zro <<= bit_xpl_nbr_zro;
                    /* Bit Set   mask for OR:  Put ones into bits to be set,
                     * zeros in untouched bits */
                    msk_f64_u64_one = ~msk_f64_u64_zro;
                    msk_f64_u64_hshv = msk_f64_u64_one &
                                       (msk_f64_u64_zro >>
                                        1); /* Set one bit: the MSB of LSBs */
                    u64_ptr[idx] +=
                        msk_f64_u64_hshv; /* Add 1 to the MSB of LSBs, carry 1
...
nc4_get_fill_value(NC_FILE_INFO_T *h5, NC_VAR_INFO_T *var, void **fillp)
{
    ...
    if (var->fill_value) {
        ...
        if (var->type_info->nc_type_class == NC_VLEN) {
            ...
            if ((retval = nc4_get_typelen_mem(
                     h5, var->type_info->u.v.base_nc_typeid, &basetypesize)))
                return retval;
...
nc4_check_chunksizes(
    NC_GRP_INFO_T *grp, NC_VAR_INFO_T *var, const size_t *chunksizes)
{
    double dprod;
    size_t type_len;
    int d;
    ...
    if ((retval = nc4_get_typelen_mem(
             grp->nc4_info, var->type_info->hdr.id, &type_len)))
        return retval;
    if (var->type_info->nc_type_class == NC_VLEN)
        dprod = (double)sizeof(nc_vlen_t);
    else
        dprod = (double)type_len;
    for (d = 0; d < var->ndims; d++) dprod *= (double)chunksizes[d];
...
nc4_find_default_chunksizes2(NC_GRP_INFO_T *grp, NC_VAR_INFO_T *var)
{
    ...
    for (d = 0; d < var->ndims; d++) {
        ...
        if (!var->dim[d]->unlimited)
            ...
            num_unlim++;
            var->chunksizes[d] =
                1; /* overwritten below, if all dims are unlimited */
        }
    }
    ...
    if (var->ndims == 1 && num_unlim == 1) {
        ...
        LOG(
            (4,
             "%s: name %s dim %d DEFAULT_CHUNK_SIZE %d num_values %f "
             "type_size %d "
             "chunksize %ld",
             __func__,
             var->hdr.name,
    ...
    if (var->ndims > 1 &&
        (float)var->ndims == num_unlim) { /* all dims unlimited */
        suggested_size = (size_t)pow(
            (double)DEFAULT_CHUNK_SIZE / (double)type_size,
            1.0 / (double)(var->ndims));
        for (d = 0; d < var->ndims; d++) {
    ...
    for (d = 0; d < var->ndims; d++)
        if (!var->chunksizes[d]) {
            suggested_size =
                (size_t)(pow((double)DEFAULT_CHUNK_SIZE /
                                 (num_values * (double)type_size),
                             1.0 / (double)((double)var->ndims - num_unlim)) *
                             (double)var->dim[d]->len -
                         .5);
    ...
    if (retval) {
        ...
        for (; retval == NC_EBADCHUNK;
             retval = nc4_check_chunksizes(grp, var, var->chunksizes))
            for (d = 0; d < var->ndims; d++)
                var->chunksizes[d] =
                    var->chunksizes[d] / 2 ? var->chunksizes[d] / 2 : 1;
    }
    ...
    for (d = 0; d < var->ndims; d++) {
        ...
        num_chunks =
            (var->dim[d]->len + var->chunksizes[d] - 1) / var->chunksizes[d];
        if (num_chunks > 0) {
...
nc4_get_default_fill_value(NC_TYPE_INFO_T *tinfo, void *fill_value)
{
    ...
    switch (tinfo->nc_type_class) {
        case NC_ENUM:
            return nc4_get_default_atomic_fill_value(
                tinfo->u.e.base_nc_typeid, fill_value);
        case NC_OPAQUE:
        case NC_VLEN:
        case NC_COMPOUND:
            if (fill_value)
