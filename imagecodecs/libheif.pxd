...
cdef extern from 'libheif/heif.h' nogil:
    ...
    void heif_string_release(
        const char*
    )

    ctypedef struct heif_init_params:
        int version

    heif_error heif_init(
        heif_init_params*
    )
    ...
    ctypedef struct heif_plugin_info:
        int version
        heif_plugin_type type_
        const void* plugin
    ...
    ctypedef struct heif_decoder_plugin:
        pass

    ctypedef struct heif_encoder_plugin:
        pass
    ...
    int LIBHEIF_NUMERIC_VERSION
    char* LIBHEIF_VERSION
    char* LIBHEIF_PLUGIN_DIRECTORY

    # libheif/heif_error.h
    ...
    ctypedef struct heif_reading_options:
        pass
    ...
    ctypedef struct heif_security_limits:
        pass
    ...
    heif_error heif_image_extract_area(
        const heif_image*,
        uint32_t x0,
        uint32_t y0,
        uint32_t w,
    ...
    const uint8_t* heif_image_get_plane_readonly2(
        const heif_image*,
        heif_channel channel,
        size_t* out_stride
    )

    uint8_t* heif_image_get_plane2(
        heif_image*,
        heif_channel channel,
        size_t* out_stride
    )

    ctypedef struct heif_scaling_options:
        pass
    ...
    void heif_image_handle_set_pixel_aspect_ratio(
        heif_image_handle*,
        uint32_t aspect_h,
        uint32_t aspect_v
    )
    ...
    heif_error heif_image_add_plane_safe(
        heif_image* image,
        heif_channel channel,
        int width,
        int height,
    ...
    heif_error heif_image_extend_padding_to_size(
        heif_image* image,
        int min_physical_width,
        int min_physical_height
    )

    # libheif/heif_color.h
    ...
    ctypedef struct heif_color_conversion_options:
        uint8_t version
    ...
    ctypedef struct heif_color_conversion_options_ext:
        uint8_t version
        ...
        uint16_t secondary_background_red
        uint16_t secondary_background_green
        uint16_t secondary_background_blue
    ...
    void heif_color_conversion_options_ext_copy(
        heif_color_conversion_options_ext* dst,
        const heif_color_conversion_options_ext* src
    )

    void heif_color_conversion_options_ext_free(
        heif_color_conversion_options_ext*
    )
    ...
    ctypedef struct heif_content_light_level:
    ...
    void heif_image_set_content_light_level(
        const heif_image*,
        const heif_content_light_level* in_
    )

    void heif_image_handle_set_content_light_level(
        const heif_image_handle*,
        const heif_content_light_level* in_
    )

    ctypedef struct heif_mastering_display_colour_volume:
    ...
    ctypedef struct heif_decoded_mastering_display_colour_volume:
    ...
    ctypedef struct heif_ambient_viewing_environment:
    ...
    void heif_image_set_mastering_display_colour_volume(
        const heif_image*,
        const heif_mastering_display_colour_volume* in_
    )

    void heif_image_handle_set_mastering_display_colour_volume(
        const heif_image_handle*,
        const heif_mastering_display_colour_volume* in_
    )
    ...
    int heif_metadata_compression_method_supported(
        heif_metadata_compression method
    )
    ...
    ctypedef struct heif_depth_representation_info:
        uint8_t version
        uint8_t has_z_near
        uint8_t has_z_far
        uint8_t has_d_min
    ...
    # libheif/heif_entity_groups.h

    ctypedef uint32_t heif_entity_group_id

    ctypedef struct heif_entity_group:
    ...
    # libheif/heif_encoding.h

    ctypedef struct heif_encoder:
        pass

    ctypedef struct heif_encoder_descriptor:
        pass

    ctypedef struct heif_encoder_parameter:
        pass
    ...
    const heif_encoder_parameter** heif_encoder_list_parameters(
        heif_encoder*
    )
    ...
    void heif_encoding_options_copy(
        heif_encoding_options* dst,
        const heif_encoding_options* src
    )
    ...
    void heif_context_set_major_brand(
        heif_context* ctx,
    ...
    ctypedef struct heif_decoder_descriptor:
        pass
    ...
    const char* heif_decoder_descriptor_get_name(
        const heif_decoder_descriptor*
    )

    const char* heif_decoder_descriptor_get_id_name(
        const heif_decoder_descriptor*
    )
    ...
    const char* heif_image_handle_get_gimi_content_id(
        const heif_image_handle* handle
    )

    # libheif/heif_tiling.h

    ctypedef struct heif_image_tiling:
        int version
